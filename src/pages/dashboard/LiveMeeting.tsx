...
import { FirstTimeGuide, FIRST_TIME_GUIDE_STORAGE_KEY } from "../../components/FirstTimeGuide";
...
export default function LiveMeeting() {
  ...
  useEffect(() => {
    ...
    const fetchMeeting = async () => {
      try {
        ...
        console.error("Failed to load meeting:", msg);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  // Show first-time guide on first visit to Live Meeting (when we have a loaded meeting)
  useEffect(() => {
    if (isLoading || !meeting) return;
    try {
      if (!localStorage.getItem(FIRST_TIME_GUIDE_STORAGE_KEY)) {
        setShowGuide(true);
      }
    } catch {
  ...
  const handleSpeech = async (text: string, options: any) => {
    try {
      ...
      const msg = err instanceof Error ? err.message : String(err);
      console.error("Failed to handle speech:", msg);
    }
  };
  ...
  const handleLeaveMeeting = async () => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : String(err);
      console.error("Failed to update meeting:", msg);
    }
  ...
  return (
    <div className="flex h-screen">
      ...
      <FirstTimeGuide open={showGuide} onComplete={() => setShowGuide(false)} />
